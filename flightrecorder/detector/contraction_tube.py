...
AND CUSUM on a geometry signal (two-of-N), SUSTAINED for `persistence` steps so a
transient spike cannot trigger it. Fit on warmup window or an external reference."""
...
class ContractionTubeDetector:
    def __init__(self, warmup: int = 20, rho: float = 3.0, tau: float = 1.0,
        ...
        self._S = 0.0; self._dwell = 0
        self._onset_step: int | None = None; self._n = 0

    def reset(self) -> None:
        self.__init__(self.warmup, self.rho, self.tau, self.k, self.h,
                      self.cusum_signal, self.persistence)
    ...
    def update(self, frame: RolloutFrame) -> DetectorState:
        ...
        self._dwell = self._dwell + 1 if (tube_hit and cusum_hit) else 0
        score = min(1.0, 0.5 * tube_distance / max(self.tau, 1e-6)
                    + 0.5 * self._S / max(self.h, 1e-6))
        if self._dwell >= self.persistence and self._onset_step is None:
            ...
            return make_state(frame.step, True, self._onset_step, max(score, 0.5),
                              ...
                              f"sustained tube-exit({tube_distance:.2f}) AND CUSUM({self._S:.2f})")
